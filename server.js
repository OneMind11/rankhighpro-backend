...
// tier — Pro customers can also reach the team directly outside this.
// ---------------------------------------------------------------------------
...
// One row per Fully Managed By Us customer who has at least one message,
// most recently active first — the inbox list.
...
app.get('/api/admin/direct-messages/:customerId', async (req, res) => {
  ...
  try {
    const customerResult = await pool.query(
      `SELECT id, email FROM customers WHERE id = $1 AND plan = 'fullservice'`,
