...
async function initDb() {
  ...
  // ---------------------------------------------------------------------
  // VIDEO ADS (HeyGen)
  // One row per talking-avatar video a customer generates. Billed through
  // Ty's own HeyGen API account (HEYGEN_API_KEY env var) — the customer
  // never sees HeyGen or gets a separate bill. Generation is asynchronous
  // on HeyGen's side (usually 1-3 minutes), so a row starts as
  // 'processing' with heygen_video_id set, and a cron job (see
  // pollProcessingVideoAds below) polls HeyGen and flips it to
  // 'completed' (with video_url) or 'failed' (with error) once it's done.
  // NOTE: HeyGen's video_url expires 7 days after generation — customers
  // should be told to download videos they want to keep.
  // ---------------------------------------------------------------------
  await pool.query(`
  ...
  // ---------------------------------------------------------------------
  // AFFILIATE PROGRAM
  // Affiliates are a separate identity from customers (a friend can be an
  // affiliate without ever subscribing themselves). Each affiliate gets a
  // referral_code baked into a link (rankhighpro.com/?ref=CODE); when
  // someone checks out through that link, affiliate_referrals ties that
  // new customer to the affiliate FOREVER (one row per customer_id — first
  // referral wins, enforced by the UNIQUE constraint below). Real money
  // moves through Stripe Connect (stripe_connect_account_id) via Stripe
  // Transfers, logged in affiliate_payouts so payouts are never duplicated
  // (see the UNIQUE constraint tied to the Stripe invoice that triggered
  // each recurring commission).
  // ---------------------------------------------------------------------
  await pool.query(`
  ...
  `);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS affiliate_magic_tokens (
      id SERIAL PRIMARY KEY,
      email TEXT NOT NULL,
      token TEXT UNIQUE NOT NULL,
      expires_at TIMESTAMPTZ NOT NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS affiliate_payouts (
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      UNIQUE(referral_id, stripe_invoice_id)
    );
  `);
  // ---------------------------------------------------------------------
...
// Same pattern as sendMagicLinkEmail, separate function only so the subject
// line/copy is affiliate-specific and this stays easy to find and edit.
async function sendAffiliateLoginEmail(email, link) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('RESEND_API_KEY not set — affiliate login link not emailed. Link:', link);
    return;
  }
  await fetch('https://api.resend.com/emails', {
    method: 'POST',
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM_EMAIL || 'RankHighPro <onboarding@resend.dev>',
      to: email,
      subject: 'Your RankHighPro affiliate portal link',
      html: `<p>Click below to view your referral link and earnings. This link expires in 15 minutes.</p><p><a href="${link}">${link}</a></p>`,
    }),
  });
}

// Sends Ty a quick heads-up email for a real-time event (new customer, new
// affiliate, a referral converting). Silently does nothing if either env
// var isn't set — never blocks or breaks the action that triggered it.
async function notifyAdmin(subject, html) {
  if (!process.env.ADMIN_NOTIFY_EMAIL || !process.env.RESEND_API_KEY) {
    console.warn('ADMIN_NOTIFY_EMAIL or RESEND_API_KEY not set — admin alert not emailed:', subject);
    return;
  }
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (email) {
            ...
            // Only alert on a genuinely brand-new signup, not someone
            // re-subscribing or updating an existing row — keeps the inbox
            // meaningful instead of noisy.
            if (upsertResult.rows[0].is_new) {
              notifyAdmin(
                `🎉 New RankHighPro signup — ${email}`,
                `<p><strong>New customer signed up</strong></p><p>Email: ${email}<br>Plan: ${plan}${referralCode ? `<br>Referred by code: ${referralCode}` : ''}</p>`
              );
            }

            // Affiliate referral — first referral for a given customer wins
            // (UNIQUE(customer_id) on affiliate_referrals), and an affiliate
            // can never be credited for referring themselves.
            if (referralCode) {
              const customerId = upsertResult.rows[0].id;
              const affResult = await pool.query(
                `SELECT id, email, name FROM affiliates WHERE referral_code = $1`,
                [referralCode]
              );
              if (affResult.rows.length > 0 && affResult.rows[0].email !== email) {
                const affiliateId = affResult.rows[0].id;
                const referralResult = await pool.query(
                  `INSERT INTO affiliate_referrals (affiliate_id, customer_id, plan)
                   VALUES ($1, $2, $3)
                   ON CONFLICT (customer_id) DO NOTHING
                   RETURNING id`,
                  [affiliateId, customerId, plan]
                );
                if (referralResult.rows.length > 0) {
                  await payAffiliateBounty(affiliateId, referralResult.rows[0].id, plan);
                  notifyAdmin(
                    `💰 Affiliate referral converted — ${affResult.rows[0].name || affResult.rows[0].email}`,
                    `<p><strong>A referral just became a paying customer</strong></p><p>Affiliate: ${affResult.rows[0].name || ''} (${affResult.rows[0].email})<br>Referred: ${email}<br>Plan: ${plan}</p>`
                  );
                }
              }
            }
        ...
        case 'invoice.payment_succeeded': {
          // Pays the affiliate's recurring commission, if this customer was
          // referred. Runs on EVERY successful invoice, including the
          // first one after a trial — see AFFILIATE_PROGRAM's comment for
          // why that's the intended behavior, not a bug.
          const invoice = event.data.object;
          const amountPaidCents = invoice.amount_paid || 0;
          if (amountPaidCents > 0) {
            const customerResult = await pool.query(
              `SELECT id, plan, status FROM customers WHERE stripe_customer_id = $1`,
              [invoice.customer]
            );
            if (customerResult.rows.length > 0) {
              const customer = customerResult.rows[0];
              const referralResult = await pool.query(
                `SELECT id, affiliate_id FROM affiliate_referrals WHERE customer_id = $1`,
                [customer.id]
              );
              if (referralResult.rows.length > 0) {
                const referral = referralResult.rows[0];
                await payAffiliateRecurringCommission({
        ...
        case 'charge.refunded': {
          // We don't auto-claw-back affiliate payouts already transferred —
          // reversing a Stripe Transfer needs funds available in the
          // affiliate's connected account, which isn't guaranteed. This just
          // flags it clearly in the logs so it can be handled by hand.
          const charge = event.data.object;
          const customerResult = await pool.query(
            `SELECT c.id FROM customers c WHERE c.stripe_customer_id = $1`,
            [charge.customer]
          );
          if (customerResult.rows.length > 0) {
            const referralResult = await pool.query(
              `SELECT affiliate_id FROM affiliate_referrals WHERE customer_id = $1`,
              [customerResult.rows[0].id]
            );
            if (referralResult.rows.length > 0) {
              console.warn(`REFUND on a referred customer (Stripe customer ${charge.customer}, affiliate ${referralResult.rows[0].affiliate_id}) — check whether any affiliate payout for this customer needs manual clawback.`);
            }
          }
          break;
        }
...
// ---------------------------------------------------------------------------
// AFFILIATE PROGRAM — auth + self-service routes.
// Affiliates are a completely separate identity from customers (someone
// can be an affiliate without ever subscribing to RankHighPro themselves),
// so this mirrors the customer magic-link auth pattern above with its own
// table and JWT shape rather than reusing the customer one.
// ---------------------------------------------------------------------------
function requireAffiliateAuth(req, res, next) {
  ...
  try {
    const payload = jwt.verify(token, JWT_SECRET);
    if (!payload.affiliateId) return res.status(401).json({ error: 'Invalid session' });
    req.affiliate = { id: payload.affiliateId, email: payload.email };
    next();
  } catch (err) {
    return res.status(401).json({ error: 'Invalid or expired session' });
  }
...
// ---------------------------------------------------------------------------
// POST /api/affiliates/signup   Body: { email, name }
// Also doubles as the "returning affiliate, log me back in" route — if the
// email already has an affiliate row, this just sends them a fresh login
// link instead of creating a second one. name is optional on a return visit.
// ---------------------------------------------------------------------------
app.post('/api/affiliates/signup', async (req, res) => {
  try {
    ...
    if (existing.rows.length === 0) {
      ...
      // but retry on the off chance rather than fail the signup.
      for (let attempt = 0; attempt < 5; attempt++) {
        code = generateReferralCode();
        const clash = await pool.query(`SELECT id FROM affiliates WHERE referral_code = $1`, [code]);
        if (clash.rows.length === 0) break;
      }
      await pool.query(
        `INSERT INTO affiliates (email, name, referral_code) VALUES ($1, $2, $3)`,
        [email, name, code]
      );
      notifyAdmin(
        `🤝 New affiliate signup — ${name || email}`,
        `<p><strong>New affiliate joined the program</strong></p><p>Name: ${name || '(not given)'}<br>Email: ${email}<br>Referral code: ${code}</p>`
      );
    } else if (name) {
      await pool.query(`UPDATE affiliates SET name = $1 WHERE email = $2`, [name, email]);
    }

    const token = crypto.randomBytes(32).toString('hex');
    const expiresAt = new Date(Date.now() + 15 * 60 * 1000);
    await pool.query(
      `INSERT INTO affiliate_magic_tokens (email, token, expires_at) VALUES ($1, $2, $3)`,
      [email, token, expiresAt]
    );

    const link = `${BACKEND_URL}/api/affiliates/verify?token=${token}`;
    await sendAffiliateLoginEmail(email, link);

    res.json({ message: 'Check your email for your affiliate portal link.' });
  } catch (err) {
...
app.get('/api/affiliates/verify', async (req, res) => {
  try {
    const { token } = req.query;
    if (!token) return res.status(400).send('Missing token');

    const result = await pool.query(
      `SELECT * FROM affiliate_magic_tokens WHERE token = $1 AND used = false AND expires_at > now()`,
      [token]
    );
    if (result.rows.length === 0) {
      return res.status(400).send('This login link is invalid or has expired. Please request a new one.');
    ...
    const magicToken = result.rows[0];
    await pool.query(`UPDATE affiliate_magic_tokens SET used = true WHERE id = $1`, [magicToken.id]);

    const affResult = await pool.query(`SELECT * FROM affiliates WHERE email = $1`, [magicToken.email]);
    ...
    res.redirect(`${FRONTEND_URL}/affiliate.html#token=${jwtToken}`);
  } catch (err) {
...
// Everything the affiliate portal needs in one call: their referral link,
// Connect status, referral list, payout history, and lifetime earnings.
// Also lazily checks Stripe for whether Connect onboarding finished (rather
// than needing a separate webhook subscription just for this), and pays
// out any bounties that were waiting on that the moment it flips to ready.
// ---------------------------------------------------------------------------
app.get('/api/affiliates/me', requireAffiliateAuth, async (req, res) => {
  try {
    const affResult = await pool.query(`SELECT * FROM affiliates WHERE id = $1`, [req.affiliate.id]);
    let affiliate = affResult.rows[0];
    if (!affiliate) return res.status(404).json({ error: 'Affiliate not found' });

    // Lazily check Stripe for onboarding completion rather than needing a
    // separate webhook subscription just for this. The moment it flips to
    // ready, sweep any bounties that were waiting on it.
    if (affiliate.stripe_connect_account_id && affiliate.stripe_connect_status !== 'active') {
      try {
        ...
        if (account.charges_enabled) {
          await pool.query(`UPDATE affiliates SET stripe_connect_status = 'active' WHERE id = $1`, [affiliate.id]);
          affiliate.stripe_connect_status = 'active';
          await sweepUnpaidBounties(affiliate.id);
        }
      } catch (e) { /* ignore — checked again next load */ }
    }

    const referralsResult = await pool.query(
      `SELECT r.id, r.plan, r.bounty_paid, r.created_at, c.email, c.status AS customer_status
    ...
    const payoutsResult = await pool.query(
      `SELECT id, referral_id, type, amount_cents, status, created_at
    ...
    const totalsResult = await pool.query(
      `SELECT COALESCE(SUM(amount_cents), 0) AS total_cents
       FROM affiliate_payouts WHERE affiliate_id = $1 AND status = 'paid'`,
      [affiliate.id]
    );
    ...
    res.json({
      ...
      referralLink: `${FRONTEND_URL}/?ref=${affiliate.referral_code}`,
      connectStatus: affiliate.stripe_connect_status,
      stats: {
        totalReferrals: referralsResult.rows.length,
        activeReferrals: referralsResult.rows.filter((r) => r.customer_status === 'active').length,
        totalEarnedCents: parseInt(totalsResult.rows[0].total_cents, 10),
      },
    ...
    res.status(500).json({ error: 'Could not load affiliate data' });
  }
});

// ---------------------------------------------------------------------------
// GET /api/affiliates/connect-link
// Creates (once) a Stripe Connect Express account for this affiliate and
// returns a fresh onboarding URL. Safe to call repeatedly — if an account
// already exists, this just issues a new Account Link for it (Stripe's
// onboarding links expire quickly, so the portal calls this fresh each
// time the affiliate taps "Connect Stripe").
// ---------------------------------------------------------------------------
app.get('/api/affiliates/connect-link', requireAffiliateAuth, async (req, res) => {
  try {
    const affResult = await pool.query(`SELECT * FROM affiliates WHERE id = $1`, [req.affiliate.id]);
    const affiliate = affResult.rows[0];
    if (!affiliate) return res.status(404).json({ error: 'Affiliate not found' });
    ...
    if (!accountId) {
      ...
      await pool.query(
        `UPDATE affiliates SET stripe_connect_account_id = $1, stripe_connect_status = 'pending' WHERE id = $2`,
    ...
    const accountLink = await stripe.accountLinks.create({
      account: accountId,
      refresh_url: `${FRONTEND_URL}/affiliate.html`,
      return_url: `${FRONTEND_URL}/affiliate.html?connected=1`,
      type: 'account_onboarding',
    });

    res.json({ url: accountLink.url });
  } catch (err) {
    console.error('affiliate connect-link error:', err.message);
...
// ---------------------------------------------------------------------------
// AFFILIATE PROGRAM
// bountyCents: a ONE-TIME payout the moment a referred customer's checkout
//   completes (paid the instant Stripe confirms the subscription started —
//   doesn't wait for the trial to end).
// recurringPercent: paid on EVERY successful invoice for as long as that
//   customer stays subscribed — including the very first one — computed
//   against whatever they actually paid that invoice (so a later upgrade
//   or a promo-code discount automatically flows through correctly).
// Every tier carries the same 20% recurring rate — including Fully
// Managed, even though that tier costs real staff time on our end, since
// that's the rate Ty chose to run.
// ---------------------------------------------------------------------------
const AFFILIATE_PROGRAM = {
  watch:       { bountyCents: 2500,  recurringPercent: 20 },
  grow:        { bountyCents: 5000,  recurringPercent: 20 },
  managed:     { bountyCents: 7500,  recurringPercent: 20 },
  fullservice: { bountyCents: 10000, recurringPercent: 20 },
};

function generateReferralCode() {
  // 6 chars, uppercase letters + digits, no 0/O/1/I to avoid confusion
  // when someone reads a code out loud or copies it by hand.
...
// Pays the one-time bounty for a referral, if it hasn't been paid yet and
// the affiliate has a working Stripe Connect account. If the affiliate
// hasn't finished Connect onboarding yet, this silently does nothing —
// sweepUnpaidBounties() catches it up the moment they finish connecting.
async function payAffiliateBounty(affiliateId, referralId, plan) {
  const program = AFFILIATE_PROGRAM[plan];
  if (!program || program.bountyCents <= 0) return;

  const referralCheck = await pool.query(
    `SELECT bounty_paid FROM affiliate_referrals WHERE id = $1`,
    [referralId]
  );
  if (referralCheck.rows.length === 0 || referralCheck.rows[0].bounty_paid) return;

  const affResult = await pool.query(`SELECT * FROM affiliates WHERE id = $1`, [affiliateId]);
  const affiliate = affResult.rows[0];
  if (!affiliate || !affiliate.stripe_connect_account_id) return;

  try {
    const transfer = await stripe.transfers.create({
      amount: program.bountyCents,
      currency: 'usd',
      destination: affiliate.stripe_connect_account_id,
      description: `RankHighPro affiliate bounty — referral #${referralId}`,
    });
    await pool.query(
      `INSERT INTO affiliate_payouts (affiliate_id, referral_id, type, amount_cents, stripe_transfer_id)
       VALUES ($1, $2, 'bounty', $3, $4)`,
      [affiliateId, referralId, program.bountyCents, transfer.id]
    );
    await pool.query(`UPDATE affiliate_referrals SET bounty_paid = true WHERE id = $1`, [referralId]);
    console.log(`Paid $${(program.bountyCents / 100).toFixed(2)} bounty to affiliate ${affiliateId} for referral ${referralId}`);
  } catch (err) {
    console.error('Affiliate bounty transfer failed (will retry when Connect finishes onboarding):', err.message);
  }
}

// Called right after an affiliate's Stripe Connect account becomes able to
// receive transfers — pays out any bounties that were earned before they
// finished onboarding.
async function sweepUnpaidBounties(affiliateId) {
  const unpaid = await pool.query(
    `SELECT id, plan FROM affiliate_referrals WHERE affiliate_id = $1 AND bounty_paid = false`,
    [affiliateId]
  );
  for (const r of unpaid.rows) {
    await payAffiliateBounty(affiliateId, r.id, r.plan);
  }
}

// Pays the recurring monthly commission for one successful invoice.
// Guarded against double-payment by checking for an existing payout row
// for this exact (referral, invoice) pair before transferring — Stripe can
// redeliver the same webhook event more than once.
async function payAffiliateRecurringCommission({ affiliateId, referralId, plan, invoiceId, amountPaidCents }) {
  const program = AFFILIATE_PROGRAM[plan];
  if (!program || program.recurringPercent <= 0 || amountPaidCents <= 0) return;

  const existing = await pool.query(
    `SELECT id FROM affiliate_payouts WHERE referral_id = $1 AND stripe_invoice_id = $2`,
    [referralId, invoiceId]
  );
  ...
  const amountCents = Math.round((amountPaidCents * program.recurringPercent) / 100);
  ...
  try {
    const transfer = await stripe.transfers.create({
      ...
      currency: 'usd',
      destination: affiliate.stripe_connect_account_id,
      description: `RankHighPro affiliate commission — referral #${referralId}, invoice ${invoiceId}`,
    });
    await pool.query(
      `INSERT INTO affiliate_payouts (affiliate_id, referral_id, type, amount_cents, stripe_invoice_id, stripe_transfer_id)
       VALUES ($1, $2, 'recurring', $3, $4, $5)
       ON CONFLICT (referral_id, stripe_invoice_id) DO NOTHING`,
      [affiliateId, referralId, amountCents, invoiceId, transfer.id]
    );
    console.log(`Paid $${(amountCents / 100).toFixed(2)} recurring commission to affiliate ${affiliateId} for referral ${referralId}`);
  } catch (err) {
    console.error('Affiliate recurring commission transfer failed:', err.message);
  }
}
...
// ---------------------------------------------------------------------------
const TIER_LIMITS = {
  watch:       { aiContentPerMonth: 2,    gbpPostsPerMonth: 4,    reviewRequestsPerMonth: null, supportChatPerMonth: 40,  extraKeywordsLimit: 0, videoAdsPerMonth: 1 },
  grow:        { aiContentPerMonth: 6,    gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: 100, extraKeywordsLimit: 3, videoAdsPerMonth: 3 },
  managed:     { aiContentPerMonth: null, gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: null, extraKeywordsLimit: 5, videoAdsPerMonth: 6 },
  // Fully Managed By Us customers don't touch most self-serve tools
  // directly (the dashboard hides them entirely — see the stripped-down
  // view keyed on plan === 'fullservice'), so most limits here are moot,
  // but videoAdsPerMonth IS used — this feature is offered to them too.
  fullservice: { aiContentPerMonth: null, gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: null, extraKeywordsLimit: 5, videoAdsPerMonth: 10 },
};

// ---------------------------------------------------------------------------
// VIDEO AD AVATARS
// Every avatar customers can pick from in the dashboard's video-ad card.
// This list is fully driven by this one object — the dashboard picker
// builds itself from GET /api/video-ads/avatars, so adding, renaming, or
// removing an avatar here is the ONLY change ever needed; no HTML edits.
//
// Each entry needs a real avatar_id + voice_id from your HeyGen account.
// Fastest way to get them: deploy this file, then visit
// https://<your-backend-url>/api/admin/heygen-avatars?secret=YOUR_ADMIN_SECRET
// — that page lists every avatar/voice you have with tap-to-copy IDs.
// Leave any entry as REPLACE_WITH_REAL_* if you're not ready to offer it
// yet — it simply won't work until filled in (clear error, nothing breaks).
// Not included: your personal "Ty" avatar — that's for RankHighPro's own
// marketing, not for customers making ads for their own businesses.
// ---------------------------------------------------------------------------
const VIDEO_AVATARS = {
  liza:      { name: 'Liza',      avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  dalia:     { name: 'Dalia',     avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  kofi:      { name: 'Kofi',      avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  kenji:     { name: 'Kenji',     avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  imara:     { name: 'Imara',     avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  kacper:    { name: 'Kacper',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  marlon:    { name: 'Marlon',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  elodie:    { name: 'Elodie',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  hana:      { name: 'Hana',      avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  elliot:    { name: 'Elliot',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  teresa:    { name: 'Teresa',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  lorenzo:   { name: 'Lorenzo',   avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  juliette:  { name: 'Juliette',  avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  obi:       { name: 'Obi',       avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  anders:    { name: 'Anders',    avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  margaret:  { name: 'Margaret',  avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  dev:       { name: 'Dev',       avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
  khady:     { name: 'Khady',     avatar_id: 'REPLACE_WITH_REAL_AVATAR_ID', voice_id: 'REPLACE_WITH_REAL_VOICE_ID' },
};
...
async function checkFeatureAllowed(customerId, feature) {
  ...
  if (feature === 'videoAds') {
    if (limits.videoAdsPerMonth === null) return { allowed: true };
    if (limits.videoAdsPerMonth === 0) {
      return {
        allowed: false,
        error: `Video ad generation isn't included in your ${plan} plan. Upgrade to Grow or higher to unlock it.`,
      };
    }
    const used = await countRows(
      `SELECT COUNT(*) FROM video_ads
       WHERE customer_id = $1 AND created_at >= date_trunc('month', now()) AND status != 'failed'`,
      [customerId]
    );
    if (used >= limits.videoAdsPerMonth) {
      return {
        allowed: false,
        error: `You've used all ${limits.videoAdsPerMonth} video ad${limits.videoAdsPerMonth === 1 ? '' : 's'} included in your ${plan} plan this month. Upgrade your plan for more.`,
      };
    }
    return { allowed: true };
  }
...
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    ...
    // If a referral code came through (from ?ref=CODE on the homepage,
    // stashed in localStorage until checkout), validate it and never let
    // someone use their own code on themselves.
    let referral_code = '';
    if (referralCode) {
      const affResult = await pool.query(
        `SELECT email FROM affiliates WHERE referral_code = $1`,
        [String(referralCode).toUpperCase()]
      );
      if (affResult.rows.length > 0 && affResult.rows[0].email !== email.trim().toLowerCase()) {
...
app.post('/api/support-chat', requireAuth, async (req, res) => {
  try {
    ...
    if (marketingContext && marketingContext !== 'businessListingSetup' && plan === 'watch') {
    ...
    const MARKETING_PLATFORM_CONTEXTS = {
      ...
      businessListingSetup: 'creating and verifying brand-new Google Business Profile and Apple Business Connect listings from scratch (this is setup help, not marketing on an existing listing)',
    };

    const systemPrompt = marketingContext === 'businessListingSetup'
      ? `You are helping a small local business owner (${plan}-tier RankHighPro customer) set up their business listings from scratch — they may need Google Business Profile, Apple Business Connect, or both. These are separate, unrelated platforms: Google Business Profile (google.com/business or the Google Business Profile app) controls Google Maps and Search; Apple Business Connect (businessconnect.apple.com or the Apple Business Connect app) controls Apple Maps, Siri, and Apple Wallet. First figure out which one they're asking about (ask if unclear, or read it from a screenshot they send), then walk them through that one step by step in plain jargon-free language, assuming they're on their phone. For Google: signing in, adding the business, choosing a category, setting location/service area, adding phone and website, and verification (postcard, phone, or email — can take a few days). For Apple: signing in with an Apple ID, searching for an existing listing Apple may have already pulled in (claim it if found, otherwise add a new place), verifying ownership (usually a phone call with a verification code), filling in category/hours/photos/description, and publishing. If they send a screenshot, look at it carefully, identify which platform it's from based on what's shown, and tell them exactly what to tap next. Be warm, patient, and encouraging — this is often the most intimidating first step for a new customer. Keep answers concise — this is a mobile chat interface.`
      : marketingContext && MARKETING_PLATFORM_CONTEXTS[marketingContext]
...
// ---------------------------------------------------------------------------
// VIDEO ADS (HeyGen)
// Starts a talking-avatar video on HeyGen's servers. Generation itself is
// asynchronous (1-3 min) — this function only KICKS OFF the job and
// returns HeyGen's video_id; the cron job below (pollProcessingVideoAds)
// checks on it later and fills in video_url once it's ready.
// Billed entirely through Ty's own HEYGEN_API_KEY — the customer never
// sees HeyGen or a separate charge.
// ---------------------------------------------------------------------------
async function startHeygenVideo(avatar, script) {
      ...
      video_inputs: [
        {
          ...
          voice: { type: 'text', input_text: script, voice_id: avatar.voice_id },
        },
      ],
      dimension: { width: 720, height: 1280 }, // vertical — matches social/UGC-style ad format
      caption: false,
      test: false, // false = real (billed, no watermark). Set true only for manual debugging.
    }),
  });

  const data = await response.json();
  if (!response.ok || data.error) {
    const message = data?.error?.message || data?.error || `HeyGen returned HTTP ${response.status}`;
    throw new Error(message);
  }
  return data.data.video_id;
}

// Checks HeyGen's status for one in-progress video and returns the
// normalized result this app cares about.
async function checkHeygenVideoStatus(heygenVideoId) {
  const response = await fetch(`https://api.heygen.com/v1/video_status.get?video_id=${heygenVideoId}`, {
    headers: { 'X-Api-Key': process.env.HEYGEN_API_KEY },
  });
  const data = await response.json();
  if (!response.ok) {
    throw new Error(`HeyGen status check returned HTTP ${response.status}`);
  }
  return data.data; // { status, video_url, thumbnail_url, error, ... }
}

// ---------------------------------------------------------------------------
// POST /api/video-ads/generate   (auth required)
// Body: { avatarKey, script, trackedListingId? }
// Kicks off a new video ad and returns immediately with a 'processing' row
// — the frontend polls GET /api/video-ads afterward to see it complete.
// ---------------------------------------------------------------------------
app.post('/api/video-ads/generate', requireAuth, async (req, res) => {
  try {
    if (!process.env.HEYGEN_API_KEY) {
      return res.status(500).json({ error: 'HEYGEN_API_KEY is not configured on the server.' });
    }

    const { avatarKey, script, trackedListingId } = req.body;
    const trimmedScript = (script || '').trim();
    if (!trimmedScript) {
      return res.status(400).json({ error: 'A script is required.' });
    }
    if (trimmedScript.length > 700) {
      return res.status(400).json({ error: 'Script is too long — please keep it under roughly 700 characters (about 60-75 seconds spoken).' });
    }
    const avatar = VIDEO_AVATARS[avatarKey];
    if (!avatar) {
      return res.status(400).json({ error: 'Please choose a valid avatar.' });
    }

    const quota = await checkFeatureAllowed(req.customer.id, 'videoAds');
    if (!quota.allowed) {
      return res.status(403).json({ error: quota.error });
    }

    // trackedListingId is optional (a customer with no listings yet can
    // still generate a general video), but if given, confirm it's theirs.
    if (trackedListingId) {
    ...
    let heygenVideoId;
    try {
      heygenVideoId = await startHeygenVideo(avatar, trimmedScript);
    } catch (err) {
      console.error('HeyGen start video error:', err.message);
      return res.status(502).json({ error: `Could not start video generation: ${err.message}` });
    }

    const result = await pool.query(
      `INSERT INTO video_ads (customer_id, tracked_listing_id, avatar_key, avatar_name, script, status, heygen_video_id)
       VALUES ($1, $2, $3, $4, $5, 'processing', $6) RETURNING *`,
      [req.customer.id, trackedListingId || null, avatarKey, avatar.name, trimmedScript, heygenVideoId]
    );

    res.json({ videoAd: result.rows[0] });
  } catch (err) {
    console.error('video-ads generate error:', err.message);
    res.status(500).json({ error: 'Could not start video generation right now.' });
  }
});

// ---------------------------------------------------------------------------
// GET /api/video-ads   (auth required)
// This customer's video history, newest first. The frontend polls this
// while any row is still 'processing'.
// ---------------------------------------------------------------------------
app.get('/api/video-ads', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM video_ads WHERE customer_id = $1 ORDER BY created_at DESC LIMIT 20`,
      [req.customer.id]
    );
    res.json({ videoAds: result.rows });
  } catch (err) {
    console.error('video-ads list error:', err.message);
    res.status(500).json({ error: 'Could not load your videos.' });
  }
});

// ---------------------------------------------------------------------------
// GET /api/video-ads/avatars   (auth required)
// Returns just the key + display name for every avatar in VIDEO_AVATARS —
// never the avatar_id/voice_id, which stay server-side only. The dashboard
// uses this to build the picker dynamically, so adding, renaming, or
// removing an avatar is a one-place edit (VIDEO_AVATARS above) with no
// HTML changes ever needed.
// ---------------------------------------------------------------------------
app.get('/api/video-ads/avatars', requireAuth, (req, res) => {
  const avatars = Object.entries(VIDEO_AVATARS).map(([key, a]) => ({ key, name: a.name }));
  res.json({ avatars });
});
...
// ---------------------------------------------------------------------------
// VIDEO AD POLLING
// HeyGen renders videos asynchronously (usually 1-3 minutes), so this runs
// every minute, checks every row still marked 'processing', and updates it
// to 'completed' (with video_url) or 'failed' (with error) once HeyGen is
// done. Keeps this cheap by skipping entirely when nothing is in flight.
// ---------------------------------------------------------------------------
async function pollProcessingVideoAds() {
  if (!process.env.HEYGEN_API_KEY) return;
  try {
    const processing = await pool.query(
      `SELECT id, heygen_video_id FROM video_ads WHERE status = 'processing' AND heygen_video_id IS NOT NULL`
    );
    if (processing.rows.length === 0) return;

    for (const row of processing.rows) {
      try {
        const heygenData = await checkHeygenVideoStatus(row.heygen_video_id);
        if (heygenData.status === 'completed') {
          await pool.query(
            `UPDATE video_ads SET status = 'completed', video_url = $1, thumbnail_url = $2, completed_at = now() WHERE id = $3`,
            [heygenData.video_url, heygenData.thumbnail_url || null, row.id]
          );
        } else if (heygenData.status === 'failed') {
          const errMsg = heygenData.error?.message || heygenData.error?.detail || 'Video generation failed on HeyGen.';
          await pool.query(
            `UPDATE video_ads SET status = 'failed', error = $1, completed_at = now() WHERE id = $2`,
            [errMsg, row.id]
          );
        }
        // 'processing' / 'pending' — leave as-is, check again next minute.
      } catch (err) {
        console.error(`video-ad poll error for video_ads row ${row.id}:`, err.message);
      }
    }
  } catch (err) {
    console.error('pollProcessingVideoAds error:', err.message);
  }
}
cron.schedule('* * * * *', pollProcessingVideoAds);
...
// ---------------------------------------------------------------------------
// ADMIN — Signups report
// Your permanent "go to" for signup activity: totals for today/this week/
// all-time on both customers and affiliates, plus the most recent of each.
// Bookmark admin.html and check this whenever you want — no email digging.
// ---------------------------------------------------------------------------
app.get('/api/admin/signups', async (req, res) => {
  ...
  try {
    const customerTotals = await pool.query(`
      ...
      FROM customers
    `);
    const affiliateTotals = await pool.query(`
      SELECT
        COUNT(*) AS all_time,
        COUNT(*) FILTER (WHERE created_at > now() - interval '1 day') AS today,
        COUNT(*) FILTER (WHERE created_at > now() - interval '7 days') AS this_week
    ...
    const recentCustomers = await pool.query(`
      SELECT email, plan, status, created_at FROM customers
      ORDER BY created_at DESC LIMIT 50
    `);
    const recentAffiliates = await pool.query(`
      SELECT name, email, referral_code, stripe_connect_status, created_at FROM affiliates
      ORDER BY created_at DESC LIMIT 50
    `);

    res.json({
      customers: {
        allTime: parseInt(customerTotals.rows[0].all_time, 10),
        today: parseInt(customerTotals.rows[0].today, 10),
        thisWeek: parseInt(customerTotals.rows[0].this_week, 10),
        activeNow: parseInt(customerTotals.rows[0].active_now, 10),
        recent: recentCustomers.rows,
      },
      affiliates: {
        allTime: parseInt(affiliateTotals.rows[0].all_time, 10),
        today: parseInt(affiliateTotals.rows[0].today, 10),
        thisWeek: parseInt(affiliateTotals.rows[0].this_week, 10),
        recent: recentAffiliates.rows,
      },
    });
  } catch (err) {
    console.error('admin signups error:', err.message);
    res.status(500).json({ error: 'Could not load signups report' });
  }
});

// ---------------------------------------------------------------------------
// ADMIN — HeyGen avatar/voice browser
// A one-off convenience page: pulls your ENTIRE HeyGen avatar and voice
// library straight from the API (using the server's own HEYGEN_API_KEY —
// nothing to type in) and shows every real avatar_id / voice_id in a
// tap-to-copy list, instead of hunting through the HeyGen app one avatar
// at a time. Same ADMIN_SECRET protection as every other admin endpoint.
// Visit: https://<your-backend-url>/api/admin/heygen-avatars?secret=YOUR_ADMIN_SECRET
// ---------------------------------------------------------------------------
app.get('/api/admin/heygen-avatars', async (req, res) => {
  ...
  if (!process.env.HEYGEN_API_KEY) {
    return res.status(500).send('HEYGEN_API_KEY is not configured on the server yet.');
  }

  try {
    const [avatarsRes, voicesRes] = await Promise.all([
      fetch('https://api.heygen.com/v2/avatars', { headers: { 'X-Api-Key': process.env.HEYGEN_API_KEY } }),
      fetch('https://api.heygen.com/v2/voices', { headers: { 'X-Api-Key': process.env.HEYGEN_API_KEY } }),
    ]);
    const avatarsData = await avatarsRes.json();
    const voicesData = await voicesRes.json();
    const avatars = avatarsData?.data?.avatars || [];
    const voices = (voicesData?.data?.voices || []).filter(v => (v.language || '').toLowerCase().includes('english'));

    const html = `
      <!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1">
      <title>HeyGen Avatar Browser</title>
      <style>
        body{font-family:-apple-system,sans-serif; background:#faf6ee; margin:0; padding:16px;}
        h1{font-size:1.3rem;} h2{font-size:1.05rem; margin-top:28px;}
        .grid{display:grid; grid-template-columns:repeat(auto-fill, minmax(150px,1fr)); gap:12px;}
        .card{background:#fff; border:1px solid #e4ddcf; border-radius:12px; padding:10px; font-size:0.8rem;}
        .card img{width:100%; border-radius:8px; margin-bottom:6px; background:#eee;}
        .name{font-weight:700; margin-bottom:4px;}
        .idbox{background:#f4efe3; border-radius:6px; padding:6px; font-family:monospace; font-size:0.7rem; word-break:break-all; -webkit-user-select:all; user-select:all;}
        .voice-row{background:#fff; border:1px solid #e4ddcf; border-radius:10px; padding:10px 12px; margin-bottom:8px; font-size:0.85rem;}
      </style></head><body>
      <h1>HeyGen Avatars (${avatars.length})</h1>
      <p style="font-size:0.8rem; color:#6b665c;">Tap-and-hold an ID box to copy it. Send me the name + ID pairs you want in the RankHighPro picker.</p>
      <div class="grid">
        ${avatars.map(a => `
          <div class="card">
            <img src="${a.preview_image_url || ''}" loading="lazy">
            <div class="name">${a.avatar_name || '(unnamed)'}</div>
            <div class="idbox">${a.avatar_id}</div>
          </div>
        `).join('')}
      </div>
      <h2>English Voices (${voices.length})</h2>
      ${voices.map(v => `
        <div class="voice-row">
          <strong>${v.name || v.language}</strong> ${v.gender ? '· ' + v.gender : ''}
          <div class="idbox">${v.voice_id}</div>
        </div>
      `).join('')}
      </body></html>
    `;
    ...
    console.error('admin heygen-avatars error:', err.message);
    res.status(500).send('Could not load HeyGen avatars: ' + err.message);
  }
});

// ---------------------------------------------------------------------------
// ADMIN — Affiliate program
// Read-only view for admin.html: who's an affiliate, what they've earned,
// and the full payout log. Same ADMIN_SECRET protection as every other
// admin endpoint.
// ---------------------------------------------------------------------------
app.get('/api/admin/affiliates', async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      SELECT
        a.id, a.name, a.email, a.referral_code, a.stripe_connect_status, a.created_at,
        COUNT(DISTINCT r.id) AS total_referrals,
        COUNT(DISTINCT r.id) FILTER (WHERE c.status = 'active') AS active_referrals,
        COALESCE(SUM(p.amount_cents) FILTER (WHERE p.status = 'paid'), 0) AS total_paid_cents
      FROM affiliates a
      LEFT JOIN affiliate_referrals r ON r.affiliate_id = a.id
      LEFT JOIN customers c ON c.id = r.customer_id
      LEFT JOIN affiliate_payouts p ON p.affiliate_id = a.id
      GROUP BY a.id
      ORDER BY total_paid_cents DESC, a.created_at DESC
    `);
...
// GET /api/admin/affiliate-payouts?secret=...
// Full payout log, most recent first — useful for reconciling against
// Stripe's own Transfers list.
app.get('/api/admin/affiliate-payouts', async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      SELECT p.id, p.type, p.amount_cents, p.status, p.stripe_transfer_id, p.created_at,
             a.name AS affiliate_name, a.email AS affiliate_email,
             c.email AS customer_email
      FROM affiliate_payouts p
      JOIN affiliates a ON a.id = p.affiliate_id
      JOIN affiliate_referrals r ON r.id = p.referral_id
      JOIN customers c ON c.id = r.customer_id
    ...
    console.error('admin affiliate-payouts error:', err.message);
