...
// ---------------------------------------------------------------------------
// LOCAL EXCLUSIVITY (Grow & Managed only)
// On Watch, direct local competitors can all sign up — no restriction. On
// Grow and Managed, we only allow ONE business per business_category per
// local area (matched on category + "City, State" location, both
// case-insensitive). The same business_category in a DIFFERENT area is
// always fine on any tier — this only blocks local, same-category overlap
// on the paid-exclusivity tiers.
// ---------------------------------------------------------------------------
...
async function checkExclusivity(customerId, businessCategory, location) {
  ...
  if (taken) {
    return {
      allowed: false,
      error: `This is exclusive — a ${businessCategory.trim()} business in ${location.trim()} already holds this spot on Grow or Managed, which means no local competitor of the same type can join RankHighPro at all right now, on any plan. Join the waitlist and we'll email and text you the moment a spot opens up.`,
    };
  }
...
app.patch('/api/social-ads/:id/reject', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `UPDATE social_ads SET status = 'rejected', decided_at = now()
       WHERE id = $1 AND tracked_listing_id IN (SELECT id FROM tracked_listings WHERE customer_id = $2)
       RETURNING *`,
      [req.params.id, req.customer.id]
    );
...
app.patch('/api/social-media-posts/:id/reject', requireAuth, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE social_media_posts SET status = 'rejected', decided_at = now() WHERE id = $1`, [req.params.id]);
...
app.patch('/api/x-posts/:id/reject', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `UPDATE x_posts SET status = 'rejected', decided_at = now()
       WHERE id = $1 AND tracked_listing_id IN (SELECT id FROM tracked_listings WHERE customer_id = $2)
       RETURNING *`,
      [req.params.id, req.customer.id]
    );
...
app.post('/api/admin/fix-customer-email', async (req, res) => {
  ...
  try {
    ...
    if (dupResult.rows.length > 0) {
      ...
      const listingsCheck = await pool.query(`SELECT COUNT(*) FROM tracked_listings WHERE customer_id = $1`, [dup.id]);
      if (parseInt(listingsCheck.rows[0].count, 10) > 0) {
        return res.status(409).json({ error: `A customer already exists at "${newEmail}" with real listings attached — refusing to auto-merge. Needs manual review.` });
      }
      await pool.query(`DELETE FROM customers WHERE id = $1`, [dup.id]);
    }

    await pool.query(`UPDATE customers SET email = $1 WHERE id = $2`, [newEmail, oldCustomer.id]);
    res.json({ fixed: true, customerId: oldCustomer.id, email: newEmail });
  } catch (err) {
