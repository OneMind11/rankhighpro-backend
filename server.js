...
async function checkExclusivity(customerId, businessCategory, location) {
  ...
  if (taken) {
    return {
      allowed: false,
      error: `This is exclusive — no 2 local competitors can join Grow or Managed in the same area. A ${businessCategory.trim()} business in ${location.trim()} already holds this spot. You can join Watch ($49/mo) instead until a slot opens up.`,
    };
  }
...
// ---------------------------------------------------------------------------
// POST /api/support-chat   (auth required, Watch & Grow only)
// Body: { message, history }
// A general-purpose AI assistant for Watch/Grow customers to ask anything
// about their listing, local SEO, or how to use RankHighPro. Managed
// customers get direct human support instead, so this is gated off for
...
app.post('/api/support-chat', requireAuth, async (req, res) => {
  try {
    ...
    if (plan === 'managed') {
      return res.status(403).json({ error: 'Managed customers get direct support from our team — reach out any time instead of using the AI assistant.' });
    }
    ...
    const systemPrompt = `You are the RankHighPro AI assistant, built into the customer's RankHighPro dashboard. RankHighPro is a local SEO platform that tracks Google Business Profile ranking, checks AI answer-engine visibility (ChatGPT, Gemini, Perplexity, Grok, Claude), generates AI content (Google Business posts, social ads, X posts), and sends automated review requests. Help this ${plan}-tier customer with questions about their local SEO, their Google Business Profile, how to use RankHighPro's dashboard features, and general small-business marketing advice. Be warm, direct, and practical. If asked something outside local SEO, marketing, or RankHighPro's features, answer briefly and steer back to how you can help their ranking. Keep answers concise — this is a mobile chat interface.`;

    const messages = [...(Array.isArray(history) ? history : []), { role: 'user', content: message }].slice(-20);
