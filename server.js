...
async function initDb() {
  ...
  // Used to enforce local exclusivity on Grow/Managed (see
  ...
  // ---------------------------------------------------------------------
  // WAITLIST
  // When someone wants Grow or Managed but a local competitor of the same
  ...
  `);
  // ---------------------------------------------------------------------
  // BACKLINK OPPORTUNITIES (Managed exclusive)
  ...
  `);
  // ---------------------------------------------------------------------
  // BEAT THIS COMPETITOR REPORTS (Managed exclusive)
  ...
  `);
  // ---------------------------------------------------------------------
  // AUTO-FIX TICKETS (Managed tier)
  // When a scheduled check on a Managed listing finds something we can
  ...
  // ticket per listing+issue at a time so daily Managed checks don't spam
  ...
  // ---------------------------------------------------------------------
  // WORDPRESS CONNECTIONS (Managed tier)
  ...
  // Lets the Managed auto-fix system apply schema markup fixes live,
  ...
  `);
  // ---------------------------------------------------------------------
  // SOCIAL MEDIA MARKETING POSTS (Grow & Managed)
  ...
  `);
  // ---------------------------------------------------------------------
  // CITATION CHECKLIST (Grow & Managed)
...
// Sends the RankHighPro team a ready-to-apply fix for a Managed customer,
...
// checklist item even on Managed, since there's nothing concrete to hand
// the team yet.
async function queueManagedAutoFix({ listingId, businessName, websiteUrl, issueType, fixDetail }) {
  try {
    ...
    const html = `
      <p><strong>Managed customer auto-fix ready to apply</strong></p>
      ...
      <p>This is a Managed-tier customer — apply this fix directly and mark it resolved once done.</p>
    `;
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `[Managed auto-fix] ${businessName} — ${issueType}`,
        html,
      }),
    });
  } catch (err) {
...
// ---------------------------------------------------------------------------
// INSTANT RANK-DROP ALERT (Grow & Managed only)
...
// selling point for Grow/Managed.
// ---------------------------------------------------------------------------
      ...
      <tr>
        <td style="background:#ffffff; border-radius:0 0 14px 14px; padding:28px;">
          <p style="font-family:Arial,Helvetica,sans-serif; font-size:15px; color:#12212E; line-height:1.6; margin:0 0 20px;">
            We caught this the moment it happened instead of waiting for your next scheduled report — that's the whole point of Grow and Managed. Log in now to see who passed you and exactly what to fix.
          </p>
...
// ---------------------------------------------------------------------------
// LOCAL EXCLUSIVITY (Grow, Managed & Fully Managed By Us only)
// On Watch, direct local competitors can all sign up — no restriction. On
// Grow, Managed, and Fully Managed By Us, we only allow ONE business per
...
async function sendWaitlistConfirmationEmail({ to, businessName, businessCategory, location }) {
  ...
  const html = `
    <p>Hi there,</p>
    <p>You're on the waitlist for the exclusive Grow/Managed spot for <strong>${businessCategory}</strong> businesses in <strong>${location}</strong>.</p>
  ...
  <div style="background:#F6F1E4; padding:32px 16px; font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" width="100%" style="max-width:560px; margin:0 auto; border-collapse:collapse;">
      ...
      <tr>
        <td style="background:#ffffff; border-radius:0 0 14px 14px; padding:28px;">
          <p style="font-family:Arial,Helvetica,sans-serif; font-size:15px; color:#12212E; line-height:1.6; margin:0 0 20px;">
            You're first in line, ${businessName}. Head back and sign up for Grow or Managed now to lock it in — it's first-come, first-served, so don't wait too long.
          </p>
...
// Grow/Managed customer's status leaves 'active' or they remove the
...
// Public — used BEFORE checkout so nobody pays for a Grow/Managed spot that
...
app.get('/api/exclusivity-check', async (req, res) => {
  try {
    ...
    // Exclusivity now applies to every tier: if a Grow/Managed competitor
...
app.post('/api/tracked-listings', requireAuth, async (req, res) => {
  try {
    const { businessName, websiteUrl, location, keyword, businessCategory } = req.body;
    ...
    // Exclusivity applies to every tier now: if a Grow/Managed competitor
    ...
    const result = await pool.query(
      `INSERT INTO tracked_listings (customer_id, business_name, website_url, location, keyword, business_category)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [req.customer.id, businessName, websiteUrl || null, location, keyword, businessCategory || null]
    );
...
const PLANS = {
  ...
  managed: { name: 'RankHighPro Beat Your Competitors', amountCents: 39900 },
...
// ---------------------------------------------------------------------------
const TIER_LIMITS = {
  watch:       { aiContentPerMonth: 2,    gbpPostsPerMonth: 4,    reviewRequestsPerMonth: null, supportChatPerMonth: 40  },
  grow:        { aiContentPerMonth: 6,    gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: 100 },
  managed:     { aiContentPerMonth: null, gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: null },
  ...
  fullservice: { aiContentPerMonth: null, gbpPostsPerMonth: null, reviewRequestsPerMonth: null, supportChatPerMonth: null },
};
...
// Note: "managed" is the Beat Your Competitors tier and "fullservice" is
...
async function fetchAndDraftReviewReplies(listing) {
  ...
  // safe to poll for within this same background job run).
...
// Grow customers see these directly in their report and dashboard. Managed
...
// ---------------------------------------------------------------------------
// POST /api/wordpress/:listingId/connect   (auth required, Managed only)
...
app.post('/api/wordpress/:listingId/connect', requireAuth, async (req, res) => {
  try {
    ...
    if (listingResult.rows[0].plan !== 'managed') {
      return res.status(403).json({ error: 'WordPress auto-fix connection is a Managed-tier feature.' });
    }
...
// tier — Managed customers can also reach the team directly outside this.
// ---------------------------------------------------------------------------
app.post('/api/support-chat', requireAuth, async (req, res) => {
  try {
    const { message, history, image, imageMediaType } = req.body;
    ...
    const data = await response.json();
    const reply = data?.content?.map((b) => b.text || '').join(' ').trim() || 'Sorry, I could not generate a response just now — please try again.';
...
// ---------------------------------------------------------------------------
// DIRECT MESSAGING (Fully Managed By Us tier only)
// Simple message log so a Fully Managed By Us customer can reach the
// RankHighPro team directly from their dashboard, since they don't use the
// self-serve tools. Every message a customer sends emails ADMIN_NOTIFY_EMAIL.
// ---------------------------------------------------------------------------
app.post('/api/direct-message', requireAuth, async (req, res) => {
  try {
    ...
    if (plan !== 'fullservice') {
      return res.status(403).json({ error: 'Direct messaging is available on the Fully Managed By Us plan.' });
    }
    ...
    if (process.env.ADMIN_NOTIFY_EMAIL && process.env.RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: process.env.RESEND_FROM_EMAIL || 'RankHighPro <onboarding@resend.dev>',
          to: process.env.ADMIN_NOTIFY_EMAIL,
          subject: `[Fully Managed] New message from ${customerEmail}`,
          html: `<p><strong>Fully Managed By Us customer message</strong></p><p>From: ${customerEmail}</p><pre style="background:#f4f4f4; padding:12px; border-radius:6px; white-space:pre-wrap; font-size:13px;">${message.trim()}</pre>`,
        }),
...
// ---------------------------------------------------------------------------
// REVIEW REPLIES (Grow & Managed) — list + dismiss
// ---------------------------------------------------------------------------
...
app.get('/api/backlink-opportunities/:listingId', requireAuth, async (req, res) => {
  try {
    ...
    const { plan } = await getCustomerPlanStatus(req.customer.id);
    if (plan !== 'managed') {
      return res.status(403).json({ error: 'Backlink-building assistance is available on the Beat Your Competitors plan.' });
    }
...
// ---------------------------------------------------------------------------
// POST /api/backlink-opportunities/:listingId/generate   (auth, Managed only)
// Body: { force: boolean }
...
TEMPLATE: [a short, ready-to-send outreach message, 60-90 words, personalized to this business, that they can copy and paste or read over the phone — friendly and specific, not generic corporate language, ending with a clear, simple ask]
...
No preamble, no numbering, no extra commentary — just the 6 blocks separated by "---".`;
    ...
    for (const block of blocks) {
      ...
      if (name && template) {
        parsed.push({ name, type: type || 'other', why: why || '', template });
      }
    }
    ...
    for (const opp of parsed) {
      await pool.query(
        `INSERT INTO backlink_opportunities (tracked_listing_id, opportunity_name, opportunity_type, why_it_helps, outreach_template)
         VALUES ($1, $2, $3, $4, $5)`,
        [req.params.listingId, opp.name, opp.type, opp.why, opp.template]
      );
    }
...
// ---------------------------------------------------------------------------
// BEAT THIS COMPETITOR (Managed exclusive)
// ---------------------------------------------------------------------------
app.get('/api/beat-competitor/:listingId/candidates', requireAuth, async (req, res) => {
  try {
    ...
    const { plan } = await getCustomerPlanStatus(req.customer.id);
    if (plan !== 'managed') {
      return res.status(403).json({ error: 'Beat This Competitor reports are available on the Beat Your Competitors plan.' });
    }
...
// ---------------------------------------------------------------------------
// POST /api/beat-competitor/:listingId/generate   (auth, Managed only)
...
app.post('/api/beat-competitor/:listingId/generate', requireAuth, async (req, res) => {
  try {
    ...
    const existing = await pool.query(
      ...
      [req.params.listingId, competitorName.trim()]
    );
...
Competitor's review count: ${compAudit.found ? compAudit.reviewCount : 'unknown'}, rating: ${compAudit.found ? compAudit.rating : 'unknown'}
...
Write a short, direct, prioritized plan (4-6 numbered steps) for exactly how THEIR business can pass THIS competitor specifically — not generic SEO advice. Reference the actual numbers above where you have them (e.g. "they have X more reviews than you — close that gap by..."). If a number is unknown for the competitor, don't guess it — just focus on what IS known. Ground every step in a real, legitimate local SEO lever (reviews, GBP completeness, website schema/content, citations, backlinks). No preamble — start directly with step 1. Keep each step to 1-2 sentences. Keep the whole thing under 180 words.`;
    ...
    if (!reportText) {
      return res.status(502).json({ error: 'Could not generate a report right now — please try again.' });
    }
    ...
    res.json({ report: upserted.rows[0], cached: false });
  } catch (err) {
    console.error('beat-competitor generate error:', err.message);
    res.status(500).json({ error: 'Could not generate this report right now.' });
  }
});

// ---------------------------------------------------------------------------
// RANKHIGHPRO-HOSTED SITES (Grow & Managed)
...
// ---------------------------------------------------------------------------
// POST /api/hosted-site/:listingId/create   (auth required, Grow & Managed)
// ---------------------------------------------------------------------------
app.post('/api/hosted-site/:listingId/create', requireAuth, async (req, res) => {
  try {
    ...
    if (listing.plan !== 'grow' && listing.plan !== 'managed') {
      return res.status(403).json({ error: 'RankHighPro-hosted websites are available on Grow and Managed.' });
    }
...
app.post('/api/resend/connect', requireAuth, async (req, res) => {
  try {
    const { plan } = await getCustomerPlanStatus(req.customer.id);
    if (plan === 'watch') {
      return res.status(403).json({ error: 'Email marketing campaigns are available on Grow and Managed plans. Upgrade to unlock this feature.' });
    }
...
// ---------------------------------------------------------------------------
// POST /api/social-media-posts/:listingId/generate   (Grow & Managed)
// Body: { goal }
// ---------------------------------------------------------------------------
app.post('/api/social-media-posts/:listingId/generate', requireAuth, async (req, res) => {
  try {
    const { plan } = await getCustomerPlanStatus(req.customer.id);
    if (plan === 'watch') {
      return res.status(403).json({ error: 'Social media marketing content is available on Grow and Managed plans. Upgrade to unlock this feature.' });
    }
...
// ---------------------------------------------------------------------------
// CITATION BUILDING (Grow & Managed)
...
app.get('/api/citations/:listingId/nap', requireAuth, async (req, res) => {
  try {
    const { plan } = await getCustomerPlanStatus(req.customer.id);
    if (plan === 'watch') {
      return res.status(403).json({ error: 'Citation building is available on Grow and Managed plans. Upgrade to unlock this feature.' });
    }
  ...
  try {
    ...
    for (const listing of result.rows) {
      ...
      try {
        ...
        // Review replies — Grow & Managed only. Runs on the same schedule
        ...
        // Instant drop alert — Grow/Managed only, fires outside the normal
