...
- Include a light, natural call to action (e.g. "Stop by," "Order today," "Call now")
...
app.post('/api/gbp-posts/:listingId/generate', requireAuth, async (req, res) => {
  try {
    ...
    const content = await generateGbpPostContent(listingResult.rows[0]);

    const result = await pool.query(
      `INSERT INTO gbp_posts (tracked_listing_id, content) VALUES ($1, $2) RETURNING *`,
      [req.params.listingId, content]
    );
