...
function buildRankReportEmailHtml({
  ...
  position, reviewCount, rating, competitors, findings, dashboardUrl,
}) {
  ...
  const findingRows = (findings || []).length > 0
    ? findings.map((f) => `
        <tr>
          <td style="padding:6px 0; font-family:Arial,Helvetica,sans-serif; font-size:14px; color:#12212E; vertical-align:top;">
    ...
    <table role="presentation" width="100%" style="max-width:560px; margin:0 auto; border-collapse:collapse;">
      ...
      <tr>
        <td style="background:#ffffff; border-radius:0 0 14px 14px; padding:28px;">
          ...
          <table role="presentation" width="100%" style="border-collapse:collapse; margin-bottom:24px;">${competitorRows}</table>
          ` : ''}
...
async function sendRankCheckReportEmail({
  ...
  position, reviewCount, rating, competitors, findings,
}) {
  if (!process.env.RESEND_API_KEY) {
      ...
      try {
        ...
        try {
          ...
          await sendRankCheckReportEmail({
            ...
            findings: gbp.found ? [...(gbp.findings || []), ...(gbp.premiumFindings || [])] : [],
          });
        } catch (emailErr) {
