...
async function runRankCheck(keyword, location, businessName, placeId) {
  ...
  // Prefer an exact match on the business's unique Google place ID when we
  // have one — immune to naming variations, spelling, or duplicate
  // listings that a name-text match can misfire on. Falls back to the
  // name-based match below if no place ID is on file yet, or the pack
  // items don't carry a matching identifier field.
  if (placeId) {
    const idx = localPackItems.findIndex((i) => i.place_id === placeId || i.cid === placeId);
    if (idx !== -1) position = idx + 1;
  }

  if (position === null && businessName) {
