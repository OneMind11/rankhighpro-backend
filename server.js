...
app.post('/api/support-chat', requireAuth, async (req, res) => {
  try {
    ...
    if (marketingContext && marketingContext !== 'gbpSetup' && marketingContext !== 'appleBusinessSetup' && plan === 'watch') {
    ...
    const MARKETING_PLATFORM_CONTEXTS = {
      ...
      gbpSetup: 'creating and verifying a brand-new Google Business Profile listing from scratch (this is setup help, not marketing on an existing listing)',
      appleBusinessSetup: 'creating and verifying a brand-new Apple Business Connect listing from scratch (this is setup help, not marketing on an existing listing)',
    };

    const systemPrompt = marketingContext === 'gbpSetup'
      ? `You are helping a small local business owner (${plan}-tier RankHighPro customer) create a brand-new Google Business Profile listing from scratch — they don't have one yet. Walk them through google.com/business or the Google Business Profile app step by step, in plain jargon-free language, assuming they're on their phone. Cover: signing in, adding the business, choosing a category, setting location/service area, adding phone and website, and verification (postcard, phone, or email — and that it can take a few days). If they send a screenshot of the Google Business Profile app or site, look at it carefully and tell them exactly what to tap next based on what's actually shown. Be warm, patient, and encouraging — this is often the most intimidating first step for a new customer. Keep answers concise — this is a mobile chat interface.`
      : marketingContext === 'appleBusinessSetup'
      ? `You are helping a small local business owner (${plan}-tier RankHighPro customer) create or claim a brand-new Apple Business Connect listing from scratch — this is separate and different from Google, and controls how the business appears in Apple Maps, Siri, and Apple Wallet. Walk them through businessconnect.apple.com or the Apple Business Connect app step by step, in plain jargon-free language, assuming they're on their phone. Cover: signing in with an Apple ID, searching for an existing listing Apple may have already pulled in (claim it if found, otherwise add a new place), verifying ownership (usually a phone call with a verification code), filling in category/hours/photos/description, and publishing. If they send a screenshot of the Apple Business Connect app or site, look at it carefully and tell them exactly what to tap next based on what's actually shown. Be warm, patient, and encouraging. Keep answers concise — this is a mobile chat interface.`
